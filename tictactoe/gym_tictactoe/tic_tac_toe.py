...
class TicTacToeEnv(gym.Env):
    ...
    def __init__(self, symbols, board_size=3, win_size=3):
        ...
        self.symbols = {symbols[0]: "x", symbols[1]: "o"}
        self.action_space = spaces.Discrete(self.board_size * self.board_size)
        self.observation_space = spaces.Discrete(self.board_size * self.board_size)
        settings_file = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "settings.xml"
        )
    ...
    def check_horizontal(self):
        ...
        for i in range(0, self.board_size * self.board_size, self.board_size):
            ...
            for j in range(1, self.board_size):
                (cnt, k) = (
                    (cnt + 1, k)
                    if (grid[k] == grid[i + j] and grid[k] != 0)
                    else (0, i + j)
                )
    ...
    def check_matrix(self, m):
        ...
        for i in range(self.win_size):
            for j in range(self.win_size):
                ...
                if (
                    i + j == self.win_size - 1
                    and m[0][self.win_size - 1] == m[i][j]
                    and m[0][self.win_size - 1] != 0
                ):
