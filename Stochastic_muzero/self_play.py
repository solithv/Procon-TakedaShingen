...
def learning_cycle(
    ...
    assert (
        isinstance(number_of_iteration, int) and number_of_iteration >= 1
    ), "number_of_iterationt ∈ int | {1 < number_of_iteration < +inf)"
    assert (
        isinstance(number_of_self_play_before_training, int)
        and number_of_self_play_before_training >= 0
    ), "number_of_self_play_before_training ∈ int | {0 < number_of_self_play_before_training < +inf)"
    assert (
        isinstance(number_of_training_before_self_play, int)
        and number_of_training_before_self_play >= 0
    ), "number_of_training_before_self_play ∈ int | {0 < number_of_training_before_self_play < +inf)"
    assert (
        isinstance(model_tag_number, int) and model_tag_number >= 0
    ), "model_tag_number ∈ int | {0 < model_tag_number < +inf)"
    assert (
        isinstance(number_of_worker_selfplay, int) and number_of_worker_selfplay >= 0
    ), "number_of_worker_selfplay ∈ float | {0 < discount < +inf)"
    assert isinstance(temperature_type, str) and temperature_type in [
        "reversal_tanh_temperature",
        "extreme_temperature",
        "linear_decrease_temperature",
        "static_temperature",
        "static_one_temperature",
    ], "temperature_type ∈ {reversal_tanh_temperature,extreme_temperature,linear_decrease_temperature,static_temperature,static_one_temperature} ⊆ str "
    ...
    if number_of_worker_selfplay >= 2:
        ray.init(
            num_cpus=number_of_worker_selfplay,
            num_gpus=torch.cuda.device_count(),
            include_dashboard=False,
        )

    for ep in tqdm.tqdm(range(1, number_of_iteration + 1), desc="episode"):
        ...
        if number_of_worker_selfplay >= 2:
                ...
                [
                    play_game_ray.remote(
                        ...
                        temperature=temperature_scheduler(
                            number_of_iteration + 1, ep, mode=temperature_type
                        ),
            ...
            game = [
                ...
                for _ in tqdm.tqdm(
                    range(number_of_self_play_before_training),
                    leave=False,
                    desc="before",
                )
            ]
        ...
        did_better = (
            None
            if reward[-1] == max(reward) and not all(g.reanalyzed for g in game)
            else "do not save"
        )
        ...
        for _ in tqdm.tqdm(
            range(number_of_training_before_self_play), leave=False, desc="train"
        ):
            new_priority, batch_game_position = muzero_model.train(
...
def play_game_from_checkpoint(
    ...
    from monte_carlo_tree_search import MinMaxStats, Monte_carlo_tree_search, Node
    ...
    if render:
        try:
            env = gym.make(game_to_play, render_mode="human")
        except:
            env = gym.make(game_to_play, render_mode="rgb_array")
    else:
        env = gym.make(game_to_play)

    # # # initialize model class without initializing a neural network
    muzero = Muzero(load=True, type_format=model_type)
    ...
    for counter in range(game_iter):
        ...
        state = gameplay.observation(
            iteration=counter, feedback=observation_reward_done_info
        )
        ...
        tree = mcts.run(
            observation=state, model=muzero, train=mcts_with_or_without_dirichlet_noise
        )

        # # # select the best action from policy and inject the action into the game (.step())
        observation_reward_done_info = gameplay.policy_step(
            root=tree, temperature=temperature, feedback=None, iteration=counter
        )
...
def benchmark(model_tag, reward, action, policy, folder="report", verbose=False):
    ...
    with open(
        f"{folder}/model_{model_tag[0]}_action_and_policy_benchmark.txt", "a+"
    ) as f:
...
def report(
    muzero, replay_buffer, epoch_pr, loss, reward, folder="report", verbose=False
):
    ...
    with open(f"{folder}/model_{q}_data_of_parameter_weight_and_epoch.txt", "a+") as f:
        ...
        print(
            "|||||||||||||| Preview MODEL WEIGHT OF <representation_function> ||||||||||||||",
            file=f,
        )
        for i in muzero.representation_function.parameters():
            print(i, i.size(), file=f)
        print(
            "|||||||||||||| END MODEL WEIGHT OF <representation_function> ||||||||||||||||",
            file=f,
        )
        print(
            "|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||",
        ...
        print(
            "|||||||||||||| Preview MODEL WEIGHT OF <dynamics_function> ||||||||||||||||||||",
            file=f,
        )
        for i in muzero.dynamics_function.parameters():
            print(i, i.size(), file=f)
        print(
            "|||||||||||||| END MODEL WEIGHT OF <dynamics_function> ||||||||||||||||||||||",
            file=f,
        )
        print(
            "|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||",
        ...
        print(
            "|||||||||||||| Preview MODEL WEIGHT OF <prediction_function> ||||||||||||||||||",
            file=f,
        )
        for i in muzero.prediction_function.parameters():
            print(i, i.size(), file=f)
        print(
            "|||||||||||||| END MODEL WEIGHT OF <prediction_function> ||||||||||||||||||||",
            file=f,
        )
        print(
            "|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||",
    ...
    all_loss = np.array(
        [[a.cpu().detach().numpy() for a in x[:]] for x in muzero.store_loss],
        dtype=np.float64,
    )
    fig, ax = plt.subplots()
    plt.plot(all_loss)
...
def generate_config_file(
    ...
    if env != None and muzero != None:
        try:
            buffer_path = replay_buffer.reanalyze_stack[1].load_path
        except:
            buffer_path = None
        dict_env = {
            "human_demonstration_buffer_builder": {
                ...
                "path_to_store_game": buffer_path
                if buffer_path is not None
                else f"config/{learning_configuration['model_tag_number']}_hbuffer.pickle",
            }
        }
    ...
    if env != None:
        try:
            rendermode = (
                env.spec.kwargs["render_mode"]
                if env.spec.kwargs["render_mode"] != None
                else None
            )
        except:
            rendermode = None
        dict_env = {"game": {"env": env.spec.id, "render": rendermode}}
        list_holder.append(dict_env)
    ...
    if muzero != None:
        dict_model = {
            "muzero": {
                "model_structure": muzero.model_structure,
    ...
    if replay_buffer != None:
        dict_buffer = {
            "replaybuffer": {
                "window_size": replay_buffer.window_size,
    ...
    if gameplay != None:
        dict_gameplay = {
            "gameplay": {"limit_of_game_play": gameplay.limit_of_game_play}
        }
    ...
    if learning_configuration != None:
        dict_lc = {
            "learning_cycle": {
                "number_of_iteration": learning_configuration["number_of_iteration"],
                ...
                "number_of_worker_selfplay": learning_configuration[
    ...
    if not None in [muzero, mcts, gameplay, env, learning_configuration]:
        ...
        dict_playgame = {
            "play_game_from_checkpoint": {
                "model_tag": learning_configuration["model_tag_number"],
    ...
    if len(list_holder) != 0:
        ...
        if learning_configuration != None:
            with open(
                f"config/experiment_{learning_configuration['model_tag_number']}_config.json",
                "w",
            ) as f:
    ...
    if save_codebase:
        ...
        zip_file = zipfile.ZipFile(
            f"config/experiment_{learning_configuration['model_tag_number']}_codebase.zip",
            "w",
        )
...
def human_demonstration_buffer_builder(
    ...
    try:
        rendermode = gym.make("CartPole-v1", render_mode=None).metadata["render_modes"][
            0
        ]
    ...
    action_space = Gym_space_transform(
        bin=number_of_bin_action, mode=mode_of_bin_action
    )
...
def Show_specific_gym_env_structure(env):
    env = gym.make(env)
    print(
        f"{env} :| observation space: {env.observation_space} | action space : {env.action_space} |"
    )
