...
class Game:
    def __init__(
        ...
        assert (
            isinstance(discount, float) and discount >= 0
        ), "discount ∈ float | {0 < discount < +inf)"
        self.limit_of_game_play = limit_of_game_play
        assert (
            isinstance(limit_of_game_play, (float, int)) and limit_of_game_play >= 0
        ), "limit_of_game_play ∈ int || float | {1 < limit_of_game_play < +inf)"
        self.action_space_size = action_dimension
        assert (
            isinstance(action_dimension, int) and action_dimension >= 1
        ), "action_dimension ∈ float | {1 < action_dimension < +inf)"
        ...
        assert (
            isinstance(priority_scale, (float, int)) and 0 <= priority_scale <= 1
        ...
        shape = (
            observation_dimension[:-1] if type(observation_dimension) == tuple else None
        )  # (24,24)
        if shape != None:
            self.transform_rgb = transforms.Compose(
                [
                    lambda x: x.copy().astype(
                        np.uint8
                    ),  # make a copy of the array and change type to uint8(allow the next transform to rescale)
    ...
    def flatten_state(self, state):
        if isinstance(state, tuple):
            state = torch.tensor(
                [i.tolist() for i in state if isinstance(i, np.ndarray)],
                dtype=torch.float,
            ).flatten()[None, ...]
        elif isinstance(state, list):
            state = torch.tensor(state, dtype=torch.float).flatten()[None, ...]
        elif isinstance(state, np.ndarray):
            state = torch.tensor(state.tolist(), dtype=torch.float).flatten()[None, ...]
        else:
            try:
                state = torch.tensor(
                    [float(i) for i in state], dtype=torch.float
                ).flatten()[None, ...]
            except:
                state = torch.tensor([float(state)], dtype=torch.float).flatten()[
                    None, ...
                ]
        return state
    ...
    def store_search_statistics(self, root):
        ...
        if visit_count.sum() >= 3:
            ...
            policy = np.array(
                [root.children[u].prior for u in list(root.children.keys())],
                dtype=np.float64,
            )
    ...
    def policy_action_reward_from_tree(self, root):
        ...
        policy = np.array(
            [root.children[u].visit_count for u in list(root.children.keys())],
            dtype=np.float64,
        )
        if policy.sum() <= 1:
            policy = np.array(
        ...
        reward = np.array(
            [root.children[u].reward for u in list(root.children.keys())],
            dtype=np.float64,
        )
    ...
    def policy_step(self, root=None, temperature=0, feedback=None, iteration=0):
        ...
        if isinstance(feedback, (tuple, type(None))):
            ...
            if self.rgb_observation:
                try:
                    ...
                    observation = self.transform_rgb(step_output[0])
            else:
            ...
            step_val = [
                feedback.observations[iteration + 1],
        ...
        self.done = step_val[2] or step_val[3] if c_max_limit else False
            ...
            if current_index < len(self.root_values):
                ...
                targets.append(
                    [
                        0.0,
                        last_reward,
                        np.zeros(self.action_space_size, dtype=np.float64),
                    ]
                )  # absorbing state
    ...
    def make_priority(self, td_steps):
        ...
        for current_index in range(len(self.root_values)):
            ...
            if current_index < len(self.root_values):
                ...
                target_value.append(0)  # absorbing state
        ...
        priority_position = (
            np.abs(np.array(self.root_values) - np.array(target_value))
            ** self.priority_scale
        )
    ...
    def create_keyboard_to_map(self):
        ...
        while cond_user_permission:
            for i in range(lenght_dict):
                keyboard = input(
                    f" The keyboard you want to set with {dict_map[i]} : (Write keyboard key and press ENTER) "
                )
            ...
            if "stop" not in keyboard.lower():
                keyboard = input(
                    f"Are you satify with this set up ( Y / N ): {dict_keyboard_map} "
                )
                if "y" in keyboard.lower():
                    cond_user_permission = False

                    path_name = input(
                        "Povide a name for the saved keyboard map at path: config/NAME_keyboard_map.json : "
                    )
    ...
    def load_keymap(self, filename_keyboard_map=None):
        # open keyboardmap
        if filename_keyboard_map is None:
            filename_keyboard_map = self.keyboard_map_path

        filename_keyboard_map = (
            filename_keyboard_map[:-5]
            if ".json" in filename_keyboard_map
            else filename_keyboard_map
        )
    ...
    def play_record(self, set_default_noop=None):
        ...
        try:
            noop_available = "NOOP" in self.env.unwrapped.get_action_meanings()
        except:
        ...
        while not self.done:
            ...
            if noop_available:
                ...
                if event[0] in self.keyboard_keys:
                    ...
                    if set_default_noop == "random":
                        set_default_noop = self.keyboard_map[
                            random.randint(0, self.keyboard_len)
                        ]
                    else:
            ...
            self.root_values.append(
                step_val[1] * self.discount ** (len(self.rewards) - 1)
            )
        ...
        self.env.close()
        print(
            f"| End simulation | score: {sum(self.rewards)} , number of action : {len(self.rewards)}"
        )
