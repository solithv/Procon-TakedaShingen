...
def main(cli_input):
    ...
    config_mode_benchmark = list(
        filter(lambda s: "benchmark" in s, cli_input_to_lower_case)
    )
    # check if benchmark argument has been provide in the cli command
    config_mode_human_buffer = list(
        filter(lambda s: "human_buffer" in s, cli_input_to_lower_case)
    )
    ...
    if len(config_mode_human_buffer) == 0:
        if len(config_directory_and_file) == 0:
            raise Exception(
                "Specify a config directory and folder such as: config/config_file.json \
                            Example : \
        ...
        if len(config_mode_play + config_mode_train + config_mode_report) == 0:
            raise Exception(
                "Specify a mode such as : train , train report , play , benchmark or any of this combination \
                            Example : \
                                python muzero_cli.py train config/config_file.json \
                                python muzero_cli.py train report config/config_file.json \
                                python muzero_cli.py train report play config/config_file.json \
    ...
    if len(config_mode_human_buffer) > 0:
        human_demonstration_buffer_builder(
            ...
            if "vision" in config["muzero"]["model_structure"]
            else False,
            keyboard_map_filename=config["human_demonstration_buffer_builder"][
                "keyboard_map_filename"
            ],
            set_default_noop=config["human_demonstration_buffer_builder"][
                "set_default_noop"
            ],
            path_to_store_game=config["human_demonstration_buffer_builder"][
                "path_to_store_game"
            ],
        )
    ...
    if len(config_mode_train) > 0:
        ...
        np.random.seed(
            config["random_seed"]["np_random_seed"]
        )  # set the random seed of numpy
        torch.manual_seed(
            config["random_seed"]["torch_manual_seed"]
        )  # set the random seed of pytorch

        # # # init/set muzero model for training and inference
        muzero = Muzero(
            model_structure=config["muzero"][
                "model_structure"
            ...
            state_space_dimensions=config["muzero"][
                "state_space_dimensions"
            ],  # support size / encoding space
            hidden_layer_dimensions=config["muzero"][
                "hidden_layer_dimensions"
            ],  # number of weight in the recursive layer of the mlp
            number_of_hidden_layer=config["muzero"][
                "number_of_hidden_layer"
            ],  # number of recusion layer of hidden layer of the mlp
            k_hypothetical_steps=config["muzero"][
            ...
            learning_rate=config["muzero"][
                "learning_rate"
            ],  # learning rate of the optimizer
            loss_type=config["muzero"]["loss_type"],
            num_of_epoch=config["muzero"][
                "num_of_epoch"
            ],  # number of step during training (the number of step of self play and training can be change)
            device=config["muzero"][
                "device"
            ],  # device on which you want the comput to be made : "cpu" , "cuda:0" , "cuda:1" , etc
            ...
            use_amp=config["muzero"][
                "use_amp"
            ],  # use mix precision for gpu (not implement yet)
            scaler_on=config["muzero"][
                "scaler_on"
            ],  # scale gradient to reduce computation
            bin_method=config["muzero"][
                "bin_method"
            ],  # "linear_bin" , "uniform_bin" : will have a regular incrementation of action or uniform sampling(pick randomly) from the bound
            bin_decomposition_number=config["muzero"][
                "bin_decomposition_number"
            ],  # number of action to sample from low/high bound of a gym discret box
            priority_scale=config["muzero"]["priority_scale"],
            rescale_value_loss=config["muzero"]["rescale_value_loss"],
        )

        if (
            config["human_demonstration_buffer_builder"]["path_to_store_game"]
            is not None
        ):
            human_buffer = DemonstrationBuffer()
            human_buffer.load_back_up_buffer(
                config["human_demonstration_buffer_builder"]["path_to_store_game"]
            )
        else:
            human_buffer = DemonstrationBuffer()
        ...
        replay_buffer = ReplayBuffer(
            window_size=config["replaybuffer"][
                "window_size"
            ],  # number of game store in the buffer
            batch_size=config["replaybuffer"][
                "batch_size"
            ],  # batch size is the number of observe game during train
            num_unroll=muzero.k_hypothetical_steps,  # number of mouve/play store inside the batched game
            td_steps=config["replaybuffer"][
                "td_steps"
            ],  # number of step the value is scale on
            game_sampling=config["replaybuffer"][
                "game_sampling"
            ...
            reanalyse_fraction=config["replaybuffer"][
                "reanalyse_fraction"
            ],  # porcentage/100 of reanalyze vs new_game
            reanalyse_fraction_mode=config["replaybuffer"][
                "reanalyse_fraction_mode"
            ],  # "chance" or "ratio"
        ...
        mcts = Monte_carlo_tree_search(
            pb_c_base=config["monte_carlo_tree_search"]["pb_c_base"],
            pb_c_init=config["monte_carlo_tree_search"]["pb_c_init"],
            discount=config["monte_carlo_tree_search"]["discount"],
            root_dirichlet_alpha=config["monte_carlo_tree_search"][
                "root_dirichlet_alpha"
            ],
            root_exploration_fraction=config["monte_carlo_tree_search"][
                "root_exploration_fraction"
            ],
            num_simulations=config["monte_carlo_tree_search"][
                "maxium_action_sample"
            ],  # number of node per level ( width of the tree )
            maxium_action_sample=config["monte_carlo_tree_search"][
                "maxium_action_sample"
            ],  # number of node per level ( width of the tree )
        ...
        gameplay = Game(
            gym_env=env,
            discount=config["monte_carlo_tree_search"][
                "discount"
            ],  # should be the same discount than mcts
            limit_of_game_play=config["gameplay"][
                "limit_of_game_play"
            ],  # maximum number of mouve
        ...
        epoch_pr, loss, reward, configuration = learning_cycle(
            number_of_iteration=config["learning_cycle"][
                "number_of_iteration"
            ],  # number of epoch(step) in  muzero should be the |total amount of number_of_iteration x number_of_training_before_self_play|
            number_of_self_play_before_training=config["learning_cycle"][
                "number_of_self_play_before_training"
            ],  # number of game played record in the replay buffer before training
            number_of_training_before_self_play=config["learning_cycle"][
                "number_of_training_before_self_play"
            ],  # number of epoch made by the model before selplay
            model_tag_number=config["learning_cycle"][
                "model_tag_number"
            ],  # tag number use to generate checkpoint
            number_of_worker_selfplay=config["learning_cycle"][
                "number_of_worker_selfplay"
            ],
            temperature_type=config["learning_cycle"][
                "temperature_type"
            ],  # "static_temperature" ,"linear_decrease_temperature" ,  "extreme_temperature" and "reversal_tanh_temperature"
            verbose=config["learning_cycle"][
                "verbose"
    ...
    if len(config_mode_play) > 0:
        ...
        play_game_from_checkpoint(
            ...
            mcts_root_dirichlet_alpha=config["monte_carlo_tree_search"][
                "root_dirichlet_alpha"
            ],
            mcts_root_exploration_fraction=config["monte_carlo_tree_search"][
                "root_exploration_fraction"
            ],
            mcts_with_or_without_dirichlet_noise=config["play_game_from_checkpoint"][
                "mcts_with_or_without_dirichlet_noise"
            ],
            number_of_monte_carlo_tree_search_simulation=config[
    ...
    if len(config_mode_benchmark) > 0:
        ...
        for _ in range(number_of_trial):
            tag, reward, action, policy = play_game_from_checkpoint(
                ...
                mcts_with_or_without_dirichlet_noise=config[
                    "play_game_from_checkpoint"
                ]["mcts_with_or_without_dirichlet_noise"],
                number_of_monte_carlo_tree_search_simulation=config[
                    "monte_carlo_tree_search"
                ...
                verbose=False,
                benchmark=True,
            )  # Need benchmark True to return output
        ...
        benchmark(cache_t, cache_r, cache_a, cache_p, folder="report", verbose=True)
        print("End benchmark")
