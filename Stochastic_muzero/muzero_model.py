...
try:
    from neural_network_mlp_model import Loss_function, l1, l2, weights_init
except:
    from .neural_network_mlp_model import Loss_function, l1, l2, weights_init
...
class Muzero:
    ...
    def reset(
        ...
        assert (
            isinstance(state_space_dimensions, int) and state_space_dimensions >= 1
        ), "state_space_dimensions ∈ int | {1 < state_space_dimensions < +inf) "
        # # # number of weight for your recursive layer
        self.hidden_layer_dimension = hidden_layer_dimensions
        assert (
            isinstance(hidden_layer_dimensions, int) and hidden_layer_dimensions >= 1
        ), "hidden_layer_dimensions ∈ int | {1 < hidden_layer_dimensions < +inf)"
        # # # Recursive layer, number of layer between your init layer and end layer
        self.number_of_hidden_layer = number_of_hidden_layer
        assert (
            isinstance(number_of_hidden_layer, int) and number_of_hidden_layer >= 0
        ), "number_of_hidden_layer ∈ int | {0 < number_of_hidden_layer < +inf)"
        # # # K future step to simulate in the forward pass and loss function
        self.k_hypothetical_steps = k_hypothetical_steps
        assert (
            isinstance(k_hypothetical_steps, int) and k_hypothetical_steps >= 0
        ), "k_hypothetical_steps ∈ int | {0 < k_hypothetical_steps < +inf)"
        # # # type of loss you want, muzero paper show a "general" and "game" loss
        # # # https://arxiv.org/pdf/1911.08265.pdf [pahe: 19]
        self.loss_type = loss_type
        assert isinstance(loss_type, str) and loss_type in [
            "general",
            "game",
        ], "loss_type ∈ {general,general_kkc,game,game_mmc) ⊆ str"
        # # # Learning rate of the optimizer
        self.lr = learning_rate
        assert (
            isinstance(learning_rate, float) and learning_rate >= 0
        ), "x ∈ float  | {0 < learning_rate < +inf)"
        # # # optimizer
        self.opt = optimizer
        assert isinstance(optimizer, str) and optimizer in [
            "adam",
            "sgd",
        ], "optimizer ∈ {sgd,adam) ⊆ str"
        # # # lr scheduler
        self.sch = lr_scheduler
        assert (
            isinstance(lr_scheduler, str) or lr_scheduler is None
        ) and lr_scheduler in [
            "steplr",
            "cosineannealinglr",
            "cosineannealinglrwarmrestarts",
            "onecyclelr",
            None,
        ], "lr_scheduler ∈ {steplr,cosineannealinglr,cosineannealinglrwarmrestarts,onecyclelr) ⊆ str"
        # # # total number of epoch that one want to compute
        self.epoch = num_of_epoch
        assert (
            isinstance(num_of_epoch, int) and num_of_epoch >= 1
        ), "num_of_epoch ∈ int | {1 < num_of_epoch < +inf) "
        # # # count the number of epoch
        self.count = 0
        assert (
            isinstance(self.count, int) and self.count == 0
        ), "self.count ∈ int | {0 ≤ self.count ≤ 0) "
        # # # The device to compute on. (CPU or GPU)
        self.device = device
        assert isinstance(device, str) and device in [
            "cpu",
            "cuda",
        ], "device ∈ {cpu,cuda) ⊆ str"
        ...
        assert isinstance(model_structure, str) and model_structure in [
            "mlp_model",
            "lstm_model",
            "vision_model",
            "vision_conv_lstm_model",
            "transformer_model",
        ], "model_structure ∈ {mlp_model,lstm_model,vision_model,vision_conv_lstm_model,transformer_model) ⊆ str"
        ...
        assert isinstance(bin_method, str) and bin_method in [
            "linear_bin",
            "uniform_bin",
        ], "bin_method ∈ {linear_bin,uniform_bin) ⊆ str"
        self.bin_decomposition_number = bin_decomposition_number
        assert (
            isinstance(bin_decomposition_number, int) and bin_decomposition_number >= 1
        ), "bin_decomposition_number ∈ int  | {1 < bin_decomposition_number < +inf)"
        self.priority_scale = priority_scale
        assert (
            isinstance(priority_scale, (int, float)) and 0 <= priority_scale <= 1
        ), "priority_scale ∈ float  | {0 < priority_scale < 1)"
        self.rescale_value_loss = rescale_value_loss
        assert (
            isinstance(rescale_value_loss, (int, float))
            and 0 <= rescale_value_loss <= 1
        ), "rescale_value_loss ∈ float  | {0 < rescale_value_loss < 1)"
        ...
        if not load:
            ...
            self.observation_dimension = self.model_obs(
                model_structure, observation_space_dimensions
            )
            ...
            action_space = Gym_space_transform(
                bin=bin_method, mode=bin_decomposition_number
            )
            ...
            assert (
                isinstance(self.action_dimension, int) and self.action_dimension >= 1
            ), "self.action_dimension ∈ int | {1 < self.action_dimension < +inf) "
        ...
        def global_imports(modulename):
            ...
            for i in model_function:
                try:
                    ...
                    context_module = __import__("Stochastic_muzero."+modulename, fromlist=[model_function])
                    globals()[i] = getattr(context_module, i)
    ...
    def model_obs(self, model_structure, observation_space_dimensions):
        if "vision" in model_structure:
            ...
            observation_dimension_per_model = self.obs_space(
                observation_space_dimensions
            )
    ...
    def model_without_amp(self):
        if not self.use_amp:
            self.representation_function = self.representation_function.type(
                self.type_format
            )
            self.prediction_function = self.prediction_function.type(self.type_format)
            self.afterstate_prediction_function = (
                self.afterstate_prediction_function.type(self.type_format)
            )
            self.afterstate_dynamics_function = self.afterstate_dynamics_function.type(
                self.type_format
            )
    ...
    def model_parallel(self):
        if torch.cuda.device_count() > 1 and self.device != "cpu":
            ...
            self.afterstate_prediction_function = torch.nn.DataParallel(
                self.afterstate_prediction_function
            )
            self.afterstate_dynamics_function = torch.nn.DataParallel(
    ...
    def init_criterion_and_optimizer(self):
        ...
        if self.loss_type == "general":
            self.criterion_value = Loss_function(
                parameter=(self.action_dimension),
                prediction=["softmax_transform", "zero_clamp_transform"],
                label=["zero_clamp_transform"],
            ).kldiv
            self.criterion_reward = Loss_function(
                parameter=(self.action_dimension),
                prediction=["softmax_transform", "zero_clamp_transform"],
                label=["zero_clamp_transform"],
            ).kldiv
            self.criterion_policy = Loss_function(
                prediction=["softmax_transform", "zero_clamp_transform"],
                label=["zero_clamp_transform"],
            ).kldiv
            self.value_afterstate_loss = Loss_function(
                prediction=["softmax_transform", "zero_clamp_transform"],
                label=["zero_clamp_transform"],
            ).kldiv
            self.distribution_afterstate_loss = Loss_function(
                prediction=["softmax_transform", "zero_clamp_transform"],
                label=["zero_clamp_transform"],
            ).kldiv
            self.vq_vae_commitment_cost = Loss_function(
                parameter=(self.action_dimension),
        ...
        self.params = [
            {"params": self.representation_function.parameters(), "lr": self.lr},
        ...
        if self.opt == "adam":
            self.optimizer = optim.Adam(
                self.params, lr=self.lr, weight_decay=0
            )  # weight_decay=1e-4 (pytorch l2 regularization)
        if self.opt == "sgd":
            self.optimizer = optim.SGD(
                self.params, lr=self.lr, momentum=0.9, weight_decay=0
            )  # weight_decay=1e-4 (pytorch l2 regularization)
        ...
        if self.sch == self.scheduler_lr[3]:
            self.scheduler = torch.optim.lr_scheduler.OneCycleLR(
                self.optimizer, max_lr=self.lr, total_steps=self.epoch
            )
    ...
    def reshape_batch(self, batches):
        ...
        X.append(
            [
                torch.cat(tuple(b[0][h] for b in batch), dim=0)
                .type(self.type_format)
                .to(self.device)
                for h in range(self.k_hypothetical_steps)
            ]
        )
        ...
        X.extend(
            torch.tensor(
                [b[1][i].tolist() for b in batch],
                dtype=self.type_format,
                device=self.device,
            )
        ...
        Y.extend(
            [
                torch.tensor(
                    [[b[2][i][0]] for b in batch],
                    dtype=self.type_format,
                    device=self.device,
                ),
                torch.tensor(
                    np.array([b[2][i][2] for b in batch]),
                    dtype=self.type_format,
                    device=self.device,
                ),  # need to fix
                torch.tensor(
                    [[b[2][i][1]] for b in batch],
                    dtype=self.type_format,
                    device=self.device,
                ),
            ]
        ...
        batch_importance_sampling_ratio = torch.tensor(
            batches[1], dtype=self.type_format, device=self.device
        )
    ...
    def one_hot_encode(self, action, counter_part):
        ...
        if not self.is_RGB:
            ...
            if len(action.size()) == 0:
                ...
                action = torch.nn.functional.one_hot(
                    action, num_classes=self.action_dimension
                ).type(self.type_format)
        ...
        if self.is_RGB:
            ...
            action = torch.cat(
                [
                    ((action_select + 1) / self.action_dimension)
                    * action_one_hot.clone()
                    for action_select in action
                ],
                dim=0,
            ).type(self.type_format)
        return action
    ...
    def training_mode(self):
        ...
        if (
            not self.representation_function.training
            or not self.dynamics_function.training
            or not self.prediction_function.training
        ):
    ...
    def transform_with_support(self, x):
        ...
        support1 = (
            torch.clamp(support1, -(shaper - 1), shaper - 1).T.clone().type(torch.int64)
        )
        weight1 = (weight1 * sign).flatten()[None].T

        support_base = (
            support_base.clone().type(weight1.dtype).scatter_(1, support1, weight1)
        )
        return support_base
    ...
    def inverse_transform_with_support(self, input):
        ...
        support_reformat = (
            support_init.expand(soft_input.shape)
            .type(soft_input.dtype)
            .to(device=soft_input.device)
        )
        y = torch.sum(support_reformat * soft_input, dim=1, keepdim=True)
        y = torch.sign(y) * (
            ((torch.sqrt(1 + 4 * 0.001 * (torch.abs(y) + 1 + 0.001)) - 1) / (2 * 0.001))
            ** 2
            - 1
        )
        return y
    ...
    def evaluate_loss(self):
        ...
        for k, (pred, target) in enumerate(zip(self.Y_pred, self.Y)):
            ...
            predict_value_k_hypothetical_steps, target_value_k_hypothetical_steps = (
                pred[0],
                target[0],
            )
            ...
            predict_policy_k_hypothetical_steps, target_policy_k_hypothetical_steps = (
                pred[1],
                target[1],
            )
            ...
            if k > 0:
                ...
                afterstate_prediction_value, afterstate_target_value = (
                    pred[3],
                    target[0],
                )
                afterstate_target_value = self.transform_with_support(
                    afterstate_target_value
                )
                loss = self.value_afterstate_loss(
                    afterstate_prediction_value, afterstate_target_value
                )
                self.rescale_gradient_and_sum_loss(
                    loss * self.rescale_value_loss, gradient_scale
                ...
                loss = self.distribution_afterstate_loss(
                    afterstate_prediction_prob, afterstate_target_prob
                )
                self.rescale_gradient_and_sum_loss(loss, gradient_scale)

                afterstate_prediction_prob, afterstate_target_prob = pred[5], pred[6]
                loss = self.vq_vae_commitment_cost(
                    afterstate_prediction_prob, afterstate_target_prob
                )
                self.rescale_gradient_and_sum_loss(loss, gradient_scale)
            ...
            self.new_priority.append(
                (
                    torch.abs(
                        torch.nan_to_num(self.inverse_transform_with_support(pred[0]))
                        - torch.nan_to_num(target[0])
                    )
                    ** self.priority_scale
                )
                .detach()
    ...
    def backpropagation(self):
        ...
        self.scaler.scale(
            self.loss_nn
        ).backward() if self.device != "cpu" and self.use_amp else self.loss_nn.backward()
    ...
    def train(self, batch):
        ...
        (
            self.X,
            self.Y,
            self.batch_importance_sampling_ratio,
            self.batch_game_position,
        ) = self.reshape_batch(batch)
        if self.use_amp:
            with torch.autocast(
                device_type=self.device, dtype=self.type_format, enabled=self.use_amp
    ...
    def prediction_function_inference(self, state_normalized):
        ...
        if self.use_amp:
            with torch.autocast(
                ...
                policy, value = self.prediction_function(state_normalized)
            if self.fp16backend:
                policy = policy.to(torch.float32)  # can't change type inside autocast
        else:
            state_normalized = self.tensor_test(state_normalized)
            policy, value = self.prediction_function(state_normalized)
        ...
        value = (
            self.inverse_transform_with_support(value)
            .detach()
            .flatten()
            .type(torch.float)
            .cpu()
    ...
    def afterstate_dynamics_function_inference(self, state_normalized, action):
        ...
        if self.use_amp:
            with torch.autocast(
                ...
                next_state_normalized = self.afterstate_dynamics_function(
                    state_normalized, one_hot_encode_action
                )
    ...
    def dynamics_function_inference(self, state_normalized, action):
        ...
        if self.use_amp:
            with torch.autocast(
                ...
                reward = (
                    self.inverse_transform_with_support(reward.type(torch.float))
                    .detach()
                    .flatten()
                    .type(torch.float)
                    .cpu()
    ...
    def save_model(
        self, directory="model_checkpoint", tag=None, model_update_or_backtrack=None
    ):
        if model_update_or_backtrack is None:
            ...
            torch.save(
                self.afterstate_prediction_function,
                f"{directory}/{self.random_tag}_muzero_afterstate_prediction_function.pt",
            )
            torch.save(
                self.afterstate_dynamics_function,
                f"{directory}/{self.random_tag}_muzero_afterstate_dynamics_function.pt",
            )
            torch.save(
                self.dynamics_function,
            ...
            torch.save(
                ...
                f"{directory}/{self.random_tag}_muzero_encoder_function.pt",
            )
            ...
            with open(
                f"{directory}/{self.random_tag}_muzero_init_variables.json", "w"
            ) as f:
    ...
    def load_model(
        ...
        with open(
            f"{model_directory}/{tag}_muzero_init_variables.json", "r"
        ) as openfile:
            init_var = json.load(openfile)

        self.reset(
            observation_space_dimensions=init_var["observation_space_dimensions"],
        ...
        self.representation_function = torch.load(
            f'{model_directory}/{init_var["random_tag"]}_muzero_representation_function.pt'
        ).to(self.device)
        self.prediction_function = torch.load(
            f'{model_directory}/{init_var["random_tag"]}_muzero_prediction_function.pt'
        ).to(self.device)
        self.afterstate_prediction_function = torch.load(
            f'{model_directory}/{init_var["random_tag"]}_muzero_afterstate_prediction_function.pt'
        ).to(self.device)
        self.afterstate_dynamics_function = torch.load(
            f'{model_directory}/{init_var["random_tag"]}_muzero_afterstate_dynamics_function.pt'
        ).to(self.device)
        self.dynamics_function = torch.load(
            f'{model_directory}/{init_var["random_tag"]}_muzero_dynamics_function.pt'
        ).to(self.device)
        self.encoder_function = torch.load(
            f'{model_directory}/{init_var["random_tag"]}_muzero_encoder_function.pt'
        ).to(self.device)
...
class Gym_space_transform:
    ...
    def continous_to_tensor(self, box_container):
        ...
        if self.mode == "uniform_bin" and "float" in str(box_container.dtype):
            box_space.extend(
                torch.distributions.uniform.Uniform(box_minmax[0], box_minmax[1])
                .sample([self.bin])
                .tolist()
                for box_minmax in overall
            )

        if self.mode == "linear_bin" and "float" in str(box_container.dtype):
            box_space.extend(
                torch.linspace(box_minmax[0], box_minmax[1], steps=self.bin).tolist()
                for box_minmax in overall
            )

        if "int" in str(box_container.dtype):
            box_space.extend(
                torch.arange(box_minmax[0], box_minmax[1], dtype=torch.int).tolist()
                for box_minmax in overall
            )
